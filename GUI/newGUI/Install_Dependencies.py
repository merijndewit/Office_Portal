...
def installRpicamsrc():
    global _stream
    _stream = subprocess.Popen(["git clone https://github.com/thaytan/gst-rpicamsrc.git"], shell=True)
    _stream = subprocess.Popen(["./autogen.sh --prefix=/usr --libdir=/usr/lib/arm-linux-gnueabihf/ && make && sudo make install"], shell=True, cwd='gst-rpicamsrc/')
